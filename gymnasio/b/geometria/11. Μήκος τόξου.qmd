...
- **Μήκος τόξου:** Για να υπολογίσουμε το μήκος ενός τόξου $l$ σε μοίρες ($\mu^\circ$), χρησιμοποιούμε τον τύπο $l = 2\pi\rho \cdot \dfrac{\mu}{360}$.
...
- **Σχέση μοιρών και ακτινίων:** Η μετατροπή μεταξύ μοιρών ($\mu$) και ακτινίων ($\alpha$) γίνεται μέσω της αναλογίας $\dfrac{\mu}{180} = \dfrac{\alpha}{\pi}$.
:::
...
$$\frac{360^o}{μ^o}=\frac{2πρ}{l}$$ Λύνοντας θα έχουμε : $l = 2\pi\rho \cdot \dfrac{\mu}{360}$.
...
<tbody>
  ...
  <tr>
    <td class="tg-0lmh">  \(360^\circ = 2\pi \) <span style="color: blue !important;">ακτίνια</span> \( (\text{ακτίνες})\)</td>
    

  <td class="tg-0lmh">\(2π\)<span style="color: blue !important;">ρ</span> \(= \textbf{2π}\) <span style="color: blue !important;">ακτίνια</span> \((rad)\)</td>
  </tr>
...
Εφαρμόζοντας τον τύπο $l = 2\pi\rho \cdot \dfrac{\mu}{360}$, προκύπτει $l = 2 \cdot 3,14 \cdot 8 \cdot \dfrac{90}{360} \approx 12,56$ cm.

\* **Παράδειγμα 2:** Εύρεση της ακτίνας όταν γνωρίζουμε το μήκος.

Αν ένα τόξο $30^\circ$ έχει μήκος $l = 1,3$ cm, τότε από τη σχέση $l = \dfrac{\pi\rho\mu}{180}$ βρίσκουμε $1,3 = 3,14 \cdot \rho \cdot \dfrac{30}{180}$, άρα $\rho \approx 2,48$ cm.

**2. Ακτίνιο (rad)**

\* **Παράδειγμα 1:** Διάταξη τόξων από το μικρότερο στο μεγαλύτερο.
Για να συγκρίνουμε τόξα όπως τα $\dfrac{\pi}{4}$ rad, $\dfrac{3\pi}{2}$ rad και $25^\circ$, τα μετατρέπουμε όλα στην ίδια μονάδα (π.χ. $\dfrac{\pi}{4} \text{ rad} = 45^\circ$).
...
Σε έναν κύκλο διπλάσιας ακτίνας $2\rho$, ένα τεταρτοκύκλιο ($\alpha = \dfrac{\pi}{2}$ rad) έχει επίσης μήκος $l = \dfrac{\pi}{2} \cdot 2\rho = \pi\rho$.

\* **Παράδειγμα 2:** Υπολογισμός ακτίνας από τόξο σε rad.
Αν ένας κυκλικός τομέας γωνίας $120^\circ$ (που ισούται με $\dfrac{2\pi}{3}$ rad) έχει μήκος τόξου $6\pi$ cm, η ακτίνα του κύκλου είναι $\rho = \dfrac{l}{\alpha} = \dfrac{6\pi}{\dfrac{2\pi}{3}} = 9$ cm.

**4. Σχέση μοιρών και ακτινίων (**$\dfrac{\mu}{180} = \dfrac{\alpha}{\pi}$)

\* **Παράδειγμα 1:** Μετατροπή από μοίρες σε ακτίνια.
Για να βρούμε πόσα rad είναι οι $225^\circ$, λύνουμε την αναλογία: $\dfrac{225}{180} = \dfrac{\alpha}{\pi}$, που δίνει $\alpha = \dfrac{5\pi}{4}$ rad.

\* **Παράδειγμα 2:** Μετατροπή από ακτίνια σε μοίρες.
Για να μετατρέψουμε τα $\dfrac{\pi}{3}$ rad σε μοίρες, έχουμε: $\dfrac{\mu}{180} = \dfrac{\dfrac{\pi}{3}}{\pi} \Rightarrow \mu = \dfrac{180}{3} = 60^\circ$.
...
2.  **Μετατροπή ακτινίων σε μοίρες** Μετέτρεψε σε μοίρες:\
    α) $\dfrac{\pi}{2}$  β) $\pi$  γ) $2\pi$  δ) $\dfrac{3\pi}{2}$  ε) $\dfrac{\pi}{3}$
...
Ένα τόξο μήκους $15\pi$ cm αντιστοιχεί σε επίκεντρη γωνία $\dfrac{5\pi}{6}$ rad.\
...
Γωνία $\theta = 120^\circ = \dfrac{2\pi}{3}$ rad \
$l_{\text{τόξου}} = ρ \cdot \theta = 12 \cdot \dfrac{2\pi}{3} = 8\pi \text{ cm}$
end{equation}

**2. Μήκος χορδής (βάση τριγώνου)** Σχεδιάζουμε το ύψος ΑΔ ==\> ΒΒ'=2ΒΔ και χρησιμοποιούμε τριγωνομρτρία $\text{χορδή} = 2ΒΔ=2ρ ημ(\dfrac{\theta}{2}) = 2 \cdot 12 \cdot ημ(60^\circ) = 24 \cdot \dfrac{\sqrt{3}}{2} = 12\sqrt{3} \text{ cm}$
...
Ένα σκαθάρι περπατάει σε κυκλική τροχιά ακτίνας 5 m κινείται από σημείο Α σε σημείο Β διαγράφοντας τόξο ($72^\circ$), μετά σε ευθεία ΒΓ μήκους 6 m, και τέλος διαγράφι ένα τόξο ΓΔ ( $\dfrac{5\pi}{6}$ ) rad.
